...
export function Ideas() {
  ...
  return (
    <>
      ...
      <div className="mb-5 flex items-start gap-3 rounded-2xl border border-warning/20 bg-warning/10 px-4 py-3.5 text-sm text-[#76540a]"><Sparkles className="mt-0.5 shrink-0" size={17} /><p><strong>Nicht sofort starten.</strong> Erst parken, dann im Review bewerten.</p></div>
      <div className="grid gap-5 xl:grid-cols-[0.85fr_1.3fr]">
        <Panel>
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            <button className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-ink px-4 py-3.5 text-sm font-bold text-white disabled:opacity-60" disabled={saving} type="submit"><Plus size={17} /> {saving ? 'Speichert …' : 'Idee parken'}</button>
          </form>
        </Panel>
        <Panel>
