  ...
  return (
    <main className="grid min-h-dvh bg-canvas lg:grid-cols-[1.1fr_0.9fr]">
      <section className="hidden bg-ink p-12 text-white lg:flex lg:flex-col lg:justify-between">
        <div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-xl bg-white text-sm font-bold text-ink">CB</span><strong className="font-display text-2xl">ControlBase</strong></div>
      ...
      <section className="flex items-center justify-center px-5 py-10 sm:px-10">
        <div className="w-full max-w-md">
          <div className="mb-10 flex items-center gap-3 lg:hidden"><span className="grid size-10 place-items-center rounded-xl bg-ink text-sm font-bold text-white">CB</span><strong className="font-display text-2xl">ControlBase</strong></div>
          ...
          <form className="mt-8 space-y-4" onSubmit={handleSubmit}>
            <label className="block"><span className="text-xs font-bold uppercase tracking-wider text-muted">E-Mail</span><input autoComplete="email" className="mt-2 w-full rounded-xl border border-line bg-white px-4 py-3.5 outline-none focus:border-accent focus:ring-2 focus:ring-accent/10" onChange={(event) => setEmail(event.target.value)} required type="email" value={email} /></label>
            <label className="block"><span className="text-xs font-bold uppercase tracking-wider text-muted">Passwort</span><input autoComplete={mode === 'login' ? 'current-password' : 'new-password'} className="mt-2 w-full rounded-xl border border-line bg-white px-4 py-3.5 outline-none focus:border-accent focus:ring-2 focus:ring-accent/10" minLength={8} onChange={(event) => setPassword(event.target.value)} required type="password" value={password} /></label>
            {error && <p className="rounded-xl bg-red-50 px-3.5 py-3 text-sm font-semibold text-red-700">{error}</p>}
            ...
            <button className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-ink px-5 py-3.5 text-sm font-bold text-white disabled:opacity-60" disabled={saving || !isSupabaseConfigured} type="submit">{saving ? 'Bitte warten …' : mode === 'login' ? 'Anmelden' : 'Konto erstellen'} <ArrowRight size={17} /></button>
          </form>
