      ...
      <div className="grid gap-5 xl:grid-cols-[0.85fr_1.3fr]">
        <Panel>
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            <button className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-control-deep px-4 py-3.5 text-sm font-bold text-white disabled:opacity-60" disabled={saving} type="submit"><Plus size={17} /> {saving ? 'Speichert …' : '7-Tage-Regel starten'}</button>
          </form>
        </Panel>

        <div className="space-y-5">
          <Panel className="bg-blue text-white"><div className="flex items-start gap-4"><span className="grid size-11 shrink-0 place-items-center rounded-xl bg-control-hover"><ShieldCheck size={21} /></span><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-white/60">Deine Regel</p><h2 className="mt-2 font-display text-2xl font-semibold">Sieben Tage zwischen Wunsch und Entscheidung.</h2><p className="mt-2 text-sm leading-6 text-white/65">Danach drei Fragen: Will ich es noch? Zahlt es auf mein Leben ein? Bleibt die Sparquote intakt?</p></div></div></Panel>
          <Panel>
