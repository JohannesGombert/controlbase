      ...
      <div className="grid gap-5 xl:grid-cols-[0.8fr_1.2fr]">
        ...
        <Panel>
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            ] as const).map(([question, key]) => <label className="block" key={key}><span className="text-xs font-bold uppercase tracking-wider text-muted">{question}</span><textarea className="mt-2 min-h-20 w-full resize-y rounded-xl border border-line bg-soft px-3.5 py-3 text-sm outline-none focus:border-accent focus:bg-white" onChange={(event) => setForm({ ...form, [key]: event.target.value })} value={form[key]} /></label>)}
            <div className="border-t border-line pt-5"><p className="mb-3 text-xs font-bold uppercase tracking-wider text-muted">Die drei Ziele für nächste Woche</p><div className="space-y-2">{(['goal1', 'goal2', 'goal3'] as const).map((key, index) => <div className="flex items-center gap-3" key={key}><span className="grid size-8 shrink-0 place-items-center rounded-full bg-ink text-xs font-bold text-white">{index + 1}</span><input className="w-full rounded-xl border border-line bg-soft px-3.5 py-3 text-sm outline-none focus:border-accent focus:bg-white" onChange={(event) => setForm({ ...form, [key]: event.target.value })} placeholder="Konkretes Ergebnis" value={form[key]} /></div>)}</div></div>
            {message && <p className="text-sm font-semibold text-positive">{message}</p>}{error && <p className="text-sm font-semibold text-red-700">{error}</p>}
            <button className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-ink px-5 py-3.5 text-sm font-bold text-white disabled:opacity-60" disabled={saving} type="submit"><Save size={17} /> {saving ? 'Speichert …' : 'Review speichern'}</button>
          </form>
        </Panel>
      </div>
    </>
